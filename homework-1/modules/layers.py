...
class Linear(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        self.output = input @ self.weight.T if self.bias is None else input @ self.weight.T + self.bias
        return self.output
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        self.mean = input.sum(axis=0) / m
        self.var = ((input - self.mean) ** 2).sum(axis=0) / m

        self.sqrt_var = np.sqrt(self.var + self.eps)
        self.input_mean = input.mean()

        self.norm_input = (input - self.mean) / self.sqrt_var #np.sqrt(self.var + self.eps)
        self.output = self.norm_input
        ...
        self.running_var = (1 - self.momentum) * self.running_var + self.momentum * m * self.var / (m - 1)
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        """
        if not self.training:
            res = grad_output / np.sqrt(self.running_var + self.eps)
            return res * self.weight if self.affine else res

        m, n = input.shape

        t = 1 / self.sqrt_var
        z = input - self.mean.reshape(1, n).repeat(m, 0)

        dt = (grad_output * z).sum(0)
        dr = dt * (-t ** 2)
        d_sig = dr * t / 2
        ds = (d_sig / m).reshape(1, n).repeat(m, axis=0)
        dz = grad_output * t.reshape(1, n).repeat(m, axis=0) + 2 * ds * z
        dm = (- dz).sum(0)

        res = dz + (dm / m).reshape(1, n).repeat(m, 0)
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        """
        if not self.training:
            self.output = deepcopy(input)
            return self.output

        n = input.shape
        self.mask = np.random.binomial(1, 1 - self.p, n)
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        for module in self.modules:
            self.output = module(self.output)
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        for i, module in enumerate(reversed(self.modules)):
            inp = self.modules[- i - 2].output if i < n - 1 else input
            grad = module.backward(inp, grad)
